...
void Model3D::processMesh(aiMesh* mesh, int meshIndex)
{
	...
	for (GLuint i = 0; i < mesh->mNumBones; i++) {
		...
		for (GLuint j = 0; j < mesh->mBones[i]->mNumWeights; j++) {
			GLuint VertexId = meshDatum[meshIndex].baseVertex 
...
void Model3D::BoneTransform
(const aiScene* modelScene, float TimeInSeconds, std::vector<aiMatrix4x4>& Transforms)
{
	aiMatrix4x4 Identity = aiMatrix4x4();

	ReadNodeHeirarchy(TimeInSeconds, modelScene->mRootNode, Identity);
...
void Model3D::ReadNodeHeirarchy
(float AnimationTime, const aiNode* pNode, const aiMatrix4x4& ParentTransform)
{
...
void Model3D::CalcInterpolatedPosition(aiVector3D& Out, float AnimationTime, const aiNodeAnim* pNodeAnim)
{
	...
	aiVector3D& Start1 = pNodeAnim->mPositionKeys[(int)AnimationTime + 1].mValue;
	const aiVector3D& End = pNodeAnim->mPositionKeys[PositionIndex + 1].mValue;
	aiVector3D Delta = End - Start;
	Out = Start + Factor * Delta;
	aiVector3D Out2 = Start;
}
